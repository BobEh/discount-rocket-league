	...
	std::vector<glm::mat4>& Offsets)
{
	...
	//float TicksPerSecond = static_cast<float>(this->mapAnimationFriendlyNameTo_pScene.find(animationName)->second.pAIScene->mAnimations[0]->mTicksPerSecond != 0 ?
	//	this->mapAnimationFriendlyNameTo_pScene.find(animationName)->second.pAIScene->mAnimations[0]->mTicksPerSecond : 25.0);
...
sModelDrawInfo* cSimpleAssimpSkinnedMesh::CreateMeshObjectFromCurrentModel(unsigned int meshIndex /*=0*/)
{
	...
	//pTheMDI->numberOfIndices = numberOfIndices;

	unsigned int index = 0;
	for (unsigned int triIndex = 0; triIndex != pTheMDI->numberOfTriangles; triIndex++, index += 3)
	{
		aiFace* pAIFace = &(pAIMesh->mFaces[triIndex]);

		pTheMDI->pIndices[index + 0] = pAIFace->mIndices[0];
		pTheMDI->pIndices[index + 1] = pAIFace->mIndices[1];
		pTheMDI->pIndices[index + 2] = pAIFace->mIndices[2];
