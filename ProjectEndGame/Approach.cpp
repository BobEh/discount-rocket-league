...
void Approach::update(float dt)
{
	...
	if (mAgent && mTarget)
	{
		distance = mTarget->getPositionXYZ() - mAgent->getPositionXYZ();
		...
		if (mAgent)
		{
			desiredVelocity = futurePosition - mAgent->getPositionXYZ();
		}
		...
		if (mTarget && mAgent)
		{
			dist = glm::distance(mTarget->getPositionXYZ(), mAgent->getPositionXYZ());
		}
		...
		if (mAgent)
		{
			steer = desiredVelocity - mAgent->getVelocity();
		}
